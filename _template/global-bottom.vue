...
/**
 * Glow effect powered by blurred, seeded polygons behind the slide content.
 * Adapted from antfu/talks (credits @pi0 @Atinux).
 *
 * Frontmatter properties read per-slide:
 * - glow: 'left' | 'right' | 'top' | 'bottom' | 'full' | 'center' | combinations like 'top-left'
 * - glowOpacity: number - opacity of the polygons (default 0.4)
 * - glowHue: number - hue shift in degrees (default 0)
...
const distribution = computed(() => (frontmatter.value.glow || false) as Distribution | false)
...
function usePoly(number = 16) {
  function getPoints(): Range[] {
    const limits = distributionToLimits(distribution.value || 'full')
